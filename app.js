...
// ── AI INSIGHT ENGINE ─────────────────────────────────────────────────────────
const AI_PROMPTS={
  week:'You are a WFM expert in E-commerce. Analyze the Weekly Overview by correlating Inflow trends with HC and Coverage. Identify the 3 most critical operational insights (e.g., understaffing due to peak surge, overstaffing inefficiencies, or %Abandon drops). Provide 1-2 actionable, data-driven recommendations categorized into Short-term (immediate scheduling tweaks) and Long-term (capacity planning). Reply in English, max 200 words, Format your response using clear bold headers and bullet points.',
  day:'You are a WFM expert in E-commerce. Analyze the Daily Detail at an interval level. Identify specific under-covered hours, diagnose the root cause (e.g., late Inflow shift, high absenteeism), and recommend precise shift adjustments (e.g., extending shifts, moving breaks). If historical event data is provided, apply a trend-multiplication logic to predict today's peak hours and justify the reasoning. Reply in English, max 200 words, Format your response using clear bold headers and bullet points.',
  shift:'You are a WFM expert in E-commerce. Using the Shift Allocation structure and the sensitivity analysis data provided (which already includes the absolute Inflow figure at each milestone), address exactly 3 points concisely: (1) Explain why this HC allocation was chosen, with its main advantages and limitations; (2) For each scenario where actual Inflow increases by 10%, 15%, 20%, and 50% — state the resulting absolute Inflow number, explain how %Task Coverage would change under the current fixed schedule, and specify exactly which shifts and how many additional staff must be ordered per shift to bring %Task Coverage back to the Coverage Target; (3) For each scenario where actual Inflow decreases by 10%, 15%, 20%, and 50% — state the resulting absolute Inflow number, and recommend exactly which shifts and how many staff could be cut per shift while still meeting the Coverage Target. Reply in English, max 320 words, Format your response using clear bold headers and bullet points.',
  trend:'You are a WFM expert in E-commerce. Analyze Inflow, HC, and Coverage trends over time to detect seasonality patterns, day-of-week effects, or data anomalies. Provide a concrete short-term outlook (next 3-7 days) forecasting potential operational bottlenecks and how to prep for them. Reply in English, max 200 words, Format your response using clear bold headers and bullet points.',
  workmode:'You are a WFM expert in E-commerce. Evaluate the WFH vs On-Floor distribution against operational needs, considering factors like flexibility for peak intervals, shrinkage, and collaboration. Assess if the current mix is cost-effective and risk-mitigated. Recommend specific percentage adjustments or staffing guardrails if misalignment is found. Reply in English, max 200 words, Format your response using clear bold headers and bullet points.'
};
